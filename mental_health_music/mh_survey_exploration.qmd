...
Now, we'll just grab the columns we need and make sure there's no missing values or anything like that. In R, we can make our `frequency_*` variables into factors as well. Each `frequency_*` variable has four possible values: "Never", "Rarely", "Sometimes", "Very frequently" to which each respondent can answer to the question "How often do you listen to * genre?"
...
freq_cols = df_mh_music_py.select(cs.starts_with("frequency")).columns
...
frequency_transformer = ColumnTransformer(
    transformers=[
        (
            "freq_ord",
            OrdinalEncoder(categories=[frequency_categories] * len(freq_cols)),
            freq_cols,
        )
    ],
...
```{python, py-workflow}
model_tree_py = model_tree_py.fit(df_feats_train_transformed, df_target_train)
workflow_py = model_tree_py.predict(df_feats_test_transformed)
...
```{r, r-grid-tuning}
df_tree_grid_r <- tibble(
  cost_complexity = c(1e-10, 1e-8, 1e-6, 1e-4, 1e-3)
)
...
param_grid = {"ccp_alpha": [1e-10, 1.78e-8, 3.16e-6, 5.62e-4, 0.1]}
grid_search = GridSearchCV(estimator=model_tree_py, param_grid=param_grid, cv=5)
grid_search.fit(df_feats_train_transformed, df_target_train)
