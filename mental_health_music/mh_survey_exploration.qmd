...
In python, there isn't a direct "factor" equivalent like in R. We can use scikit-learn's class `OrdinalEncoder` from the `sklearn.preprocessing` submodule. We do this by creating the order of the values and transforming the columns in `df_mh_music_py`. Because I'm using polars, there an extra step to get a polars dataframe back after the transformations. 
...
# Set up the transformer for ordinal encoding
...
frequency_transformer = ColumnTransformer(
    ...
    # We need to keep the anxiety column
    remainder="passthrough",
    verbose_feature_names_out=False,
)

# Need this for a polars output
frequency_transformer.set_output(transform="polars")

# Fit and transform, keeping anxiety and frequency columns
df_mh_music_py_sub = frequency_transformer.fit_transform(
    df_mh_music_py.select(["anxiety"] + freq_cols)
)
```
